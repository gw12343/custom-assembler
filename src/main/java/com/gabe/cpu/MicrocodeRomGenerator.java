...
public class MicrocodeRomGenerator {
    ...
    public static void exportROM(String path){
        ...
        addMicrocodeToAllInstruction(data, STORE_RAM | LOAD_INS | PC_COUNT, "0001");
        addMicrocodeToAllInstruction(data, MC_END, "0010");


        for(CPUInstruction instruction : CPUInstruction.values()){
            for(int i = 2; i < instruction.getMicroInstructions().size() + 2; i++){
                ...
                writeMicrocode(data, index, instruction.getMicroInstructions().get(i-2));
            }
        }
